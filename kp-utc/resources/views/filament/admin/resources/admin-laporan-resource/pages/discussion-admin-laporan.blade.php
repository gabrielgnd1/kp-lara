<x-filament-panels::page>
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h2 class="text-2xl font-bold text-gray-900">{{ $record->nama_laporan }}</h2>
            <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                    <p class="text-sm text-gray-600">Prioritas</p>
                    <p class="font-semibold">
                        <span class="px-2 py-1 rounded text-xs font-bold
                            @if($record->prioritas == 'Tinggi') bg-red-100 text-red-800
                            @elseif($record->prioritas == 'Sedang') bg-yellow-100 text-yellow-800
                ...
                <div>
                    ...
                    <p class="font-semibold">
                        <span class="px-2 py-1 rounded text-xs font-semibold
                            @if($record->decision == 'Selesai') bg-green-100 text-green-800
                            @elseif($record->decision == 'Diproses') bg-yellow-100 text-yellow-800
                            @else bg-red-100 text-red-800
                            @endif
                        ">
                            {{ $record->decision ?? 'Belum Diproses' }}
                        </span>
                    </p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Tipe</p>
                    <p class="font-semibold">{{ $record->tipe_laporan }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-600">Tanggal Lapor</p>
                    <p class="font-semibold">{{ \Carbon\Carbon::parse($record->tanggal_lapor)->format('d M Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div class="border-t border-gray-200 bg-white px-6 py-4">
                ...
                <form method="POST" action="{{ route('diskusi.store') }}" class="flex gap-2">
                    @csrf
                    <input type="hidden" name="laporan_id" value="{{ $record->id }}">
                    <input 
                        type="text"
                        name="diskusi"
