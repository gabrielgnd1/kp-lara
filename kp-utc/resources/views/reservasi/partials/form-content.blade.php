...
<div class="form-section">
    ...
    <div class="form-grid">
        <div class="form-group">
            ...
            <div class="form-value">{{ isset($reservasi['waktu_check_in']) ? \Carbon\Carbon::parse($reservasi['waktu_check_in'])->format('d-m-Y') : '-' }}</div>
        </div>
        <div class="form-group">
            <label class="form-label">Jam Check In</label>
            <div class="form-value">{{ isset($reservasi['waktu_check_in']) ? \Carbon\Carbon::parse($reservasi['waktu_check_in'])->format('H:i') : '-' }}</div>
        </div>
    </div>
    <div class="form-grid">
        <div class="form-group">
            <label class="form-label">Tanggal Check Out</label>
            <div class="form-value">{{ isset($reservasi['waktu_check_out']) ? \Carbon\Carbon::parse($reservasi['waktu_check_out'])->format('d-m-Y') : '-' }}</div>
        </div>
        <div class="form-group">
            <label class="form-label">Jam Check Out</label>
            <div class="form-value">{{ isset($reservasi['waktu_check_out']) ? \Carbon\Carbon::parse($reservasi['waktu_check_out'])->format('H:i') : '-' }}</div>
        </div>
    </div>
</div>
...
@if(isset($reservasi['pemesanan_fasilitas']) && count($reservasi['pemesanan_fasilitas']) > 0)
<div class="form-section">
    ...
    <table class="facilities-table">
        ...
        <tbody>
            @foreach($reservasi['pemesanan_fasilitas'] as $fasilitas)
            <tr>
                <td>{{ $fasilitas['nama_fasilitas'] ?? '-' }}</td>
                <td>{{ isset($fasilitas['mulai']) ? \Carbon\Carbon::parse($fasilitas['mulai'])->format('d-m-Y') : '-' }}</td>
                <td>{{ isset($fasilitas['selesai']) ? \Carbon\Carbon::parse($fasilitas['selesai'])->format('d-m-Y') : '-' }}</td>
                <td>{{ $fasilitas['jumlah_orang'] ?? "-" }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
@if(isset($reservasi['pemesanan_additional']) && count($reservasi['pemesanan_additional']) > 0)
<div class="form-section">
    ...
    <table class="items-table">
        ...
        <tbody>
            @foreach($reservasi['pemesanan_additional'] as $additional)
            <tr>
                <td>{{ $additional['nama'] ?? '-' }}</td>
                <td>{{ isset($additional['mulai']) ? \Carbon\Carbon::parse($additional['mulai'])->format('d-m-Y') : '-' }}</td>
                <td>{{ isset($additional['selesai']) ? \Carbon\Carbon::parse($additional['selesai'])->format('d-m-Y') : '-' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div class="form-section">
    ...
    <div class="form-grid">
        ...
        <div class="form-group">
            ...
            <div class="form-value">{{ isset($reservasi['tanggal_dibuat']) ? \Carbon\Carbon::parse($reservasi['tanggal_dibuat'])->format('d-m-Y H:i') : '-' }}</div>
        </div>
    </div>
</div>

<!-- Cost Details Section -->
@if(isset($reservasi['pemesanan_fasilitas']) && count($reservasi['pemesanan_fasilitas']) > 0 || isset($reservasi['pemesanan_additional']) && count($reservasi['pemesanan_additional']) > 0 || isset($reservasi['pemesanan_menu_makan']) && count($reservasi['pemesanan_menu_makan']) > 0)
<div class="form-section">
    ...
    <table class="cost-details-table">
        ...
        <tbody>
            ...
            @if(isset($reservasi['pemesanan_fasilitas']) && count($reservasi['pemesanan_fasilitas']) > 0)
                ...
                @foreach($reservasi['pemesanan_fasilitas'] as $fasilitas)
                    @php
                        ...
                        $weekday = 0;
                        $weekend = 0;
                        if (isset($fasilitas['mulai']) && isset($fasilitas['selesai'])) {
                            $mulai = \Carbon\Carbon::parse($fasilitas['mulai'])->startOfDay();
                            $selesai = \Carbon\Carbon::parse($fasilitas['selesai'])->startOfDay();
                        ...
                        $jumlahOrang = $fasilitas['jumlah_orang'] ?? 1;
                    ...
                    @endphp
                    <tr>
                        <td>{{ $fasilitas['nama_fasilitas'] ?? '-' }}{{ $jumlahOrang > 1 ? ' (' . $jumlahOrang . ' orang)' : '' }}</td>
            ...
            @if(isset($reservasi['pemesanan_additional']) && count($reservasi['pemesanan_additional']) > 0)
                ...
                @foreach($reservasi['pemesanan_additional'] as $additional)
                    @php
                        ...
                        $weekdayAdd = 0;
                        $weekendAdd = 0;
                        if (isset($additional['mulai']) && isset($additional['selesai'])) {
                            $mulaiAdd = \Carbon\Carbon::parse($additional['mulai'])->startOfDay();
                            $selesaiAdd = \Carbon\Carbon::parse($additional['selesai'])->startOfDay();
            ...
            {{-- Display Menu Makan --}}
            @if(isset($reservasi['pemesanan_menu_makan']) && count($reservasi['pemesanan_menu_makan']) > 0)
                <tr class="cost-details-category">
                    <td colspan="3" style="text-align: center;">MENU MAKAN</td>
                </tr>
                @foreach($reservasi['pemesanan_menu_makan'] as $menu)
                    @php
                        $hargaMenu = $menu['harga'] ?? 0;
                        $jumlahMenu = $menu['jumlah'] ?? 1;
...
<div class="signature-section">
    <div class="signature-box">
        ...
        <div class="signature-name">
            {{ strtoupper($reservasi['current_user_name'] ?? '( _____________________ )') }}
        </div>
    </div>
    <div class="signature-box">
