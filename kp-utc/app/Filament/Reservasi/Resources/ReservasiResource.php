    ...
    protected static function sanitizeData($data)
    {
        if (is_array($data)) {
            return array_map(function ($item) {
                return static::sanitizeData($item);
            }, $data);
        }
        
        if (is_string($data)) {
            // First detect the encoding if possible
            $encoding = mb_detect_encoding($data, ['UTF-8', 'Windows-1252', 'ISO-8859-1'], true);
            
            // If detection fails, try Windows-1252 (common for Western European text)
            if (!$encoding) {
                $encoding = 'Windows-1252';
            }
            
            // Convert to UTF-8 with strict error handling
            $converted = mb_convert_encoding($data, 'UTF-8', $encoding);
            
            // Remove any invalid UTF-8 sequences as a last resort
